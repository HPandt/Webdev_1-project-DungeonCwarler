<div class="action-panel text-center" id="game-btn">
    <div class="row">
        <div class="col-8">  
            <button class="btn btn-secondary direction-button" id="north-btn">North</button>
            <button class="btn btn-secondary direction-button" id="south-btn">South</button>
            <button class="btn btn-secondary direction-button" id="east-btn">East</button>
            <button class="btn btn-secondary direction-button" id="west-btn">West</button>
        </div>
        
    </div>
    <div class="col-4">
        <div class="col-4">
            <button class="btn btn-primary action-button" id="attack-btn">Attack</button>
        </div>
    </div>
</div>