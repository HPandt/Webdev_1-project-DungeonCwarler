document.addEventListener("DOMContentLoaded", () => {
  ...
  fetch("/api/game/start", { credentials: "same-origin" })
    .then((response) => {
      return response.text().then((text) => {
        if (!response.ok) {
          console.error("Start response not OK:", response.status, text);
          appendToLog({
            type: "danger",
            text: "Server error when starting the game. Check the console for details.",
          });
    ...
    .then((text) => {
      ...
      try {
        ...
        console.error("Start response JSON parse error:", error);
        appendToLog({
          type: "danger",
          text: "Server returned invalid JSON on game start. Check the console for details.",
        });
        throw error;
      }
      ...
      fetch("/api/game/choose-direction", {
        ...
        .then((response) => {
          return response.text().then((text) => {
            if (!response.ok) {
              console.error(
                "Choose-direction response not OK:",
                response.status,
                text,
              );
              appendToLog({
                type: "danger",
                text: "Server error when moving. Check the console for details.",
              });
              throw new Error(`HTTP ${response.status}`);
            }
            return text;
        ...
        .then((text) => {
          console.log("RAW CHOOSE-DIRECTION RESPONSE:", text);
          let data;
          try {
            data = JSON.parse(text);
          } catch (error) {
            console.error("Choose-direction JSON parse error:", error);
            appendToLog({
              type: "danger",
              text: "Server returned invalid JSON while choosing a direction. Check the console for details.",
            });
            throw error;
          }
          return data;
        ...
        .then((data) => {
          ...
          if (data.type === "exit") {
            ...
            exitButton.className = "btn btn-warning mt-3";
          ...
          if (data.monster && data.monster.name) {
            console.log(
              "Monster room - disabling movement buttons and show attack",
            );
            appendToLog({
              type: "danger",
    ...
    attackButton.addEventListener("click", () => {
      ...
      fetch("/api/game/attack", {
        ...
        .then((data) => {
          ...
          //Check if user is dead then show exit button and disable everything else
